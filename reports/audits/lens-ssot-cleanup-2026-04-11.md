...
**확인 사실**:
- 진짜 렌즈 SSOT = `/Users/ghost/Dev/nexus/shared/lenses/` + `shared/blowup/lens/` (별도 nexus 독립 프로젝트, HEXA 네이티브, `SIGMA=12.0 PHI=2.0 N=6.0 TAU=4.0 J2=24` 헤더 + `σ·φ = n·τ = J₂` 항등식 기반)
...
### 2-5. `shared/config/lens_registry.json`
...
1. **절대 금지**:
   ...
   - `lens-agent` 에이전트 호출
   - `shared/config/lens_registry.json` 에 신규 Rust 렌즈 등록
...
4. **레지스트리 재구축**: `shared/config/lens_registry.json` 진짜 SSOT 기준
