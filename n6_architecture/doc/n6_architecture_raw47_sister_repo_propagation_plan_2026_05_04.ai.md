---
schema: n6-architecture/docs/raw47_sister_repo_propagation/ai-native/1
...
> mapped against n6-architecture mk2 governance items (own#6 / own#15 / own#29 / own#14 / own#17)
...
| **n6-architecture (source)** | YES (canonical_n6_invariants*, etc) | YES | YES (.own.readme, .own.group_p) | YES | 21 | (origin) |
...
- **Migration plan (PROPOSED, not landed)**:
  ...
  - Option C — **inline the english-audit tool into n6** (copy `readme_english_audit.hexa` to `~/core/n6-architecture/tool/`): violates raw 47 cross-repo SSOT spirit (English-audit logic is hexa-lang's domain); rejected.
