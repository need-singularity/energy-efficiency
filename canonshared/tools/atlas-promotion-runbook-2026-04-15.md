...
SSOT: engine/hexa_gate_mk3.hexa + engine/ouroboros_b2_verifier.hexa + canonshared/tools/atlas_auto_promote.hexa
...
- canonshared/tools/atlas_auto_promote.hexa:244 `# fs.append_text(ATLAS_PATH, ...)` is commented out
...
hexa run canonshared/tools/atlas_auto_promote.hexa --dry-run --log-only
...
hexa run canonshared/tools/atlas_auto_promote.hexa \
...
```
# live B_2 tracking
tail -f canonshared/logs/b2_verifier.log | grep "alpha"

# track atlas.n6 SHA
while true; do shasum $NEXUS/shared/n6/atlas.n6; sleep 60; done

# count promotion log entries
wc -l canonshared/logs/atlas_auto_promote.jsonl
```
...
rm canonshared/logs/atlas_auto_promote.jsonl
