...
비교 매트릭스에 따르면 **후보 B (σ−τ=8)** 가 **최강 후보**다. 이유:
1. **n=6 에서 유일** — σ−τ=8 ⟺ (n is semiprime p·q with (p-1)(q-1)·연관 조건). 10^4 전수검색에서 (2,3)-semiprime 유일.
...
후보 A 와 C 는 **보조 정리** 로 격하하되 각자 고유한 역할을 유지한다: A 는 음악·양자화학 brideg, C 는 자기참조 고정점 정식화.
...
- n ∈ [2, 10^4] 전수검색 (standard-model-from-n6.md §2.1 기반):
  - n=6 만 (σ − τ = 8) AND (n/φ + μ = τ) AND (n squarefree) 동시 만족
  - 타 해: n=2 (τ=2, σ=3, 차=1), n=4 (τ=3, σ=7, 차=4), n=8 (τ=4, σ=15, 차=11), n=10 (τ=4, σ=18, 차=14), n=12 (τ=6, σ=28, 차=22) 등 — 모두 8 아님
  - (σ − τ) = 8 인 n: n=6, n=15 (σ=24, τ=4, 차=20 아님 — 15 재검), 추가 해 검증 필요
  - **예비 결과**: σ−τ=8 단독으론 유일 불확정, but n/φ+μ=τ 조합 조건에서 **유일**
