...
**Architecture:** Rust 모노 크레이트(`tools/nexus6/`) + metal-rs GPU 커널 + PyO3 Python 바인딩. OUROBOROS v26은 Python 확장. Discovery Graph는 JSON + Rust 분석기.

**Tech Stack:** Rust (metal, rayon, pyo3, serde, blake3, rusqlite), Metal Shading Language, Python (numpy, OUROBOROS)

**Spec:** `docs/superpowers/specs/2026-04-03-nexus6-unified-brainstorm.md`
...
**Files:**
- Create: `tools/nexus6/Cargo.toml`
- Create: `tools/nexus6/src/lib.rs`
- Create: `tools/nexus6/src/gpu/mod.rs`
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo check 2>&1 | tail -5`
Expected: `Finished` (에러 없음)

- [ ] **Step 5: 커밋**

```bash
git add tools/nexus6/
git commit -m "feat(nexus6): scaffold project + Metal device init"
```

### Task 1.2: Buffer Pool

**Files:**
- Create: `tools/nexus6/src/gpu/buffer_pool.rs`
- Test: `tools/nexus6/tests/gpu_test.rs`

- [ ] **Step 1: 테스트 작성**

```rust
// tests/gpu_test.rs
use nexus6::gpu::buffer_pool::BufferPool;
...
fn test_buffer_pool_acquire_release() {
    ...
    let device = nexus6::gpu::device().unwrap();
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo test test_buffer_pool 2>&1 | tail -10`
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo test test_buffer_pool 2>&1 | tail -5`
Expected: PASS

- [ ] **Step 5: 커밋**

```bash
git add tools/nexus6/src/gpu/buffer_pool.rs tools/nexus6/tests/gpu_test.rs
git commit -m "feat(nexus6): Metal buffer pool with acquire/release"
```

### Task 1.3: Compute Pipeline + distance_matrix kernel

**Files:**
- Create: `tools/nexus6/src/gpu/pipeline.rs`
- Create: `tools/nexus6/metal/kernels.metal`
...
fn test_distance_matrix_gpu() {
    if !nexus6::gpu::is_available() { return; }
    let device = nexus6::gpu::device().unwrap();
    ...
    let result = nexus6::gpu::dispatch_distance_matrix(device, &data, n, d);
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo test test_distance_matrix_gpu 2>&1 | tail -5`

```bash
git add tools/nexus6/metal/ tools/nexus6/src/gpu/pipeline.rs
git commit -m "feat(nexus6): Metal distance_matrix kernel + pipeline manager"
```

### Task 1.4: CPU Fallback

**Files:**
- Create: `tools/nexus6/src/gpu/fallback.rs`
...
fn test_distance_matrix_cpu_fallback() {
    let data: Vec<f32> = vec![0.0, 0.0, 3.0, 4.0, 1.0, 0.0];
    let result = nexus6::gpu::fallback::distance_matrix_cpu(&data, 3, 2);
    assert!((result[0] - 5.0).abs() < 0.01);
}
```
...
```bash
git commit -am "feat(nexus6): CPU fallback for distance_matrix"
```
...
**Files:**
- Create: `tools/nexus6/src/telescope/mod.rs`
- Create: `tools/nexus6/src/telescope/lens_trait.rs`
- Create: `tools/nexus6/src/telescope/shared_kernels.rs`
...
```bash
git commit -am "feat(nexus6): Lens trait + SharedKernelCache + Telescope API"
```

### Task 2.2: 첫 번째 렌즈 — VoidLens

**Files:**
- Create: `tools/nexus6/src/telescope/lenses/discovery.rs`
- Test: `tools/nexus6/tests/telescope_test.rs`

- [ ] **Step 1: VoidLens 테스트**

```rust
// tests/telescope_test.rs
use nexus6::telescope::lenses::discovery::VoidLens;
use nexus6::telescope::lens_trait::Lens;
use nexus6::telescope::shared_kernels::SharedKernelCache;
...
```bash
git commit -am "feat(nexus6): VoidLens — first discovery lens"
```

### Task 2.3: Tiered Scanning

**Files:**
- Create: `tools/nexus6/src/telescope/tier.rs`
...
```bash
git commit -am "feat(nexus6): TieredScanner — T0→T3 early exit"
```

### Task 2.4: Consensus System

**Files:**
- Create: `tools/nexus6/src/telescope/consensus.rs`
...
```bash
git commit -am "feat(nexus6): weighted consensus system"
```
...
**Files:**
- Create: `tools/nexus6/src/encoder/mod.rs`
- Create: `tools/nexus6/src/encoder/parser.rs`
- Create: `tools/nexus6/src/encoder/vectorize.rs`
...
- Year: 1987
"#;
    let entries = nexus6::encoder::parser::parse_hypotheses(md);
...
```bash
git commit -am "feat(nexus6): Domain Encoder — parse hypotheses.md + vectorize"
```

### Task 3.2: Materials DB

**Files:**
- Create: `tools/nexus6/src/materials/mod.rs`
- Create: `tools/nexus6/src/materials/db.rs`
- Create: `tools/nexus6/data/materials-db.json`
...
```bash
git commit -am "feat(nexus6): Materials DB loader + initial data"
```
...
**Files:**
- Create: `tools/nexus6/src/verifier/mod.rs`
- Create: `tools/nexus6/src/verifier/n6_check.rs`
- Create: `tools/nexus6/src/verifier/feasibility.rs`
...
```bash
git commit -am "feat(nexus6): Discovery Verifier — n6_check + feasibility scoring"
```
...
**Files:**
- Create: `tools/nexus6/src/graph/mod.rs`
- Create: `tools/nexus6/src/graph/node.rs`
- Create: `tools/nexus6/src/graph/edge.rs`
- Create: `tools/nexus6/src/graph/structure.rs`
- Create: `tools/nexus6/src/graph/persistence.rs`
...
```bash
git commit -am "feat(nexus6): Discovery Graph — nodes, edges, triangle detection, persistence"
```
...
**Files:**
- Create: `tools/nexus6/src/history/mod.rs`
- Create: `tools/nexus6/src/history/recorder.rs`
- Create: `tools/nexus6/src/history/stats.rs`
- Create: `tools/nexus6/src/history/recommend.rs`
...
```bash
git commit -am "feat(nexus6): Telescope History — recorder + stats + recommender"
```
...
**Files:**
- Create: `tools/nexus6/src/ouroboros/mod.rs`
- Create: `tools/nexus6/src/ouroboros/pipeline.rs`
...
#[cfg(feature = "python")]
#[pyfunction]
fn nexus6_scan(data: Vec<f64>, n: usize, d: usize) -> PyResult<String> {
...
#[cfg(feature = "python")]
#[pymodule]
fn nexus6_rs(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(nexus6_scan, m)?)?;
...
```bash
git commit -am "feat(nexus6): OUROBOROS pipeline + PyO3 bindings"
```
...
**Files:**
- Create: `tools/nexus6/src/main.rs`
...
fn main() {
    ...
    match args[1].as_str() {
        ...
        "version" => println!("NEXUS-6 v{}", nexus6::VERSION),
        _ => print_usage(),
    }
}
...
fn cmd_scan(args: &[String]) {
    ...
    println!("GPU: {}", if nexus6::gpu::is_available() { "Metal (M4)" } else { "CPU fallback" });
    // TODO: 실제 스캔 실행
}

fn cmd_run(args: &[String]) {
    println!("Starting NEXUS-6 OUROBOROS loop...");
    let mut pipeline = nexus6::ouroboros::pipeline::NexusPipeline::new();
    // TODO: 무한 루프 + 로드맵
}
...
fn cmd_benchmark() {
    ...
    println!("GPU: {}", if nexus6::gpu::is_available() { "Available" } else { "Not available" });
    // TODO: 벤치마크 실행
}
...
fn print_usage() {
    ...
    println!();
    println!("Usage: nexus6 <command> [args]");
    println!();
    println!("Commands:");
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo build --release 2>&1 | tail -3`
Run: `./target/release/nexus6 version`
Expected: `NEXUS-6 v0.1.0`

- [ ] **Step 3: 커밋**

```bash
git commit -am "feat(nexus6): CLI — scan, run, verify, graph, benchmark"
```
...
**Files:**
- Modify: `tools/nexus6/src/telescope/mod.rs`
- Create: `tools/nexus6/src/telescope/lenses/mod.rs`
...
```bash
git commit -am "feat(nexus6): lens registry + incremental lens addition pattern"
```
...
**Files:**
- Create: `tools/nexus6/tests/integration_test.rs`
...
fn test_full_pipeline_e2e() {
    ...
    // 2. Telescope 스캔
    let telescope = nexus6::telescope::Telescope::new();
    let results = telescope.scan_all(&data, n, d);
    ...
    let mut graph = nexus6::graph::persistence::DiscoveryGraph::new();
    graph.add_node(nexus6::graph::node::Node {
        id: "D-TEST-001".into(),
        node_type: nexus6::graph::node::NodeType::Discovery,
...
fn test_gpu_availability() {
    ...
    let result = nexus6::gpu::fallback::distance_matrix_cpu(&data, 2, 2);
...
Run: `cd tools/nexus6 && ~/.cargo/bin/cargo test 2>&1 | tail -10`
Expected: 모든 테스트 통과

- [ ] **Step 3: 커밋**

```bash
git commit -am "test(nexus6): integration tests — full pipeline e2e"
```
...
- [ ] **Release 빌드**: `cd tools/nexus6 && ~/.cargo/bin/cargo build --release`
- [ ] **GPU 벤치마크**: `./target/release/nexus6 benchmark`
- [ ] **첫 스캔**: `./target/release/nexus6 scan superconductor`
- [ ] **최종 커밋 + 태그**:

```bash
git tag -a v0.1.0-nexus6 -m "NEXUS-6 v0.1.0: 411-lens discovery engine with Metal GPU"
```
