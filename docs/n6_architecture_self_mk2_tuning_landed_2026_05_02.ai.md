---
schema: n6-architecture/docs/self_mk2_tuning/ai-native/1
last_updated: 2026-05-02
ssot:
  repo: /Users/<user>/core/n6-architecture
  ...
  retired_mk1_ssot: confirmed (2026-05-02; no .roadmap.* files at repo root)
related_raws:
...
|---|---|---|
| `n6shared/n6/` | n6 core constants + atlas + signals | T0 candidate (project-wide shared) |
| `n6shared/` | SSOT configs / convergence / discovery_graph / readme-data | T0 / shared-config layer |
...
`convergence/` holds 1 file (r4 propagation plan). `n6shared/convergence/n6-architecture.json` is sibling. `state/atlas_convergence_witness.jsonl` (79K) carries the run-witness stream. `experiments/anu_mc_verification/` + `experiments/_results.jsonl` are upstream feeders. Sparse but live — separate `.roadmap.auto_convergence` is justified.
...
```
/Users/<user>/core/n6-architecture/.roadmap.<domain>
```
...
|---|---|---|
| `.roadmap.n6_architecture` | meta / repo-level (entry-point pointer to all sub-roadmaps) | INDEX.json + docs/n6_meta_evolution_proposal_20260423.md |
...
```
# n6-architecture /.roadmap.<domain> — minimal seed
# Schema: n6-architecture/roadmap/<domain>/1
# Created: <ISO>
...
# upstream
- depends_on .roadmap.n6_architecture (meta-spine)
- absorbs (none) — additive only, mk1 retired

# raw#10 honest
- additive only; mk1 .roadmap retired (no migration)
...
**Triplet T1 — `n6shared/n6/`** (T0 candidate):
...
`hive/state/markers/raw_270_271_sister_repos_lint_applied.marker` indicates the hive lint already supports `--repo` arg for sister-repo scanning. n6-architecture impl path:

```
hexa run hive/tool/ai_native_module_readme_lint.hexa --repo /Users/<user>/core/n6-architecture
```
...
- path: `n6-architecture/state/markers/n6_architecture_self_mk2_tuning_landed.marker`
- format: JSON (schema `n6-architecture/markers/self_mk2_tuning_landed/1`)
...
1. **mk1 SSOT retirement scope unverified at agent-spawn**: prompt asserts "mk1 SSOT 폐기 완료" — confirmed via repo-root absence of `.roadmap` / `.roadmap.*` files. Worktree-local `.roadmap` artefacts (3 worktrees: agent-add0758622241ad36 / agent-a4a66d7e / agent-ab556f12ea2cdfd52) are NOT repo-root SSOTs.
...
3. **Tier-A roadmap count drift**: §2.1 header says "8 entries" but the table lists 9 (.roadmap.{n6_architecture, atlas, atlas_self_arch, auto_convergence, group_p, sigma_tau_8, hexa_weave, clay_millennium, honesty_triad}). Authoritative count = 9. Header is a known caveat.
...
12. **n6shared/ schema family**: `n6shared/config/absolute_rules.json` is declared SSOT per INDEX.json `rules.ssot_json`. Any roadmap that proposes lint-rule changes MUST cross-link this file (deferred).
...
| FU-3 | Run hive lint `--repo /Users/<user>/core/n6-architecture` | next-cycle | 2026-05-20 | $0 |
...
| `n6-architecture/atlas/atlas.n6` | axis-A through axis-L (read-only) |
| `n6-architecture/docs/n6_meta_evolution_proposal_20260423.md` | predecessor proposal (Phase 1-6 6-phase) |
