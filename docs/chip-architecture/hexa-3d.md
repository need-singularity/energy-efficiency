...
14. [Open Questions / TODO](#14-open-questions--todo)
15. [Links](#15-links)
...
|---|---------|----------|----------|--------|
| 1 | Thermal | Microfluidic 채널 신뢰성 (10년 수명 보장) | HIGH | 미시작 |
| 2 | Yield | 3-layer stack combined yield 50% -> 70% 경로 | HIGH | 미시작 |
| 3 | Cost | HBM4 12-hi + SoIC 3D 본딩 비용 모델 | HIGH | 미시작 |
| 4 | Testing | Known Good Die (KGD) 테스트 플로우 상세화 | MED | 미시작 |
| 5 | PIM ISA | PIM 로직의 명령어 세트 아키텍처 정의 | MED | 미시작 |
| 6 | Software | 3-tier 프로그래밍 모델 (Compute/PIM/Memory) | MED | 미시작 |
| 7 | Thermal | Thermal TSV vs Microfluidic 최적 비율 연구 | LOW | 미시작 |
| 8 | Verify | TSV 288/mm^2 밀도의 공정 feasibility 검증 | HIGH | 미시작 |
| 9 | BT | BT-75 HBM interface exponent ladder 적용 확장 | LOW | 미시작 |
| 10 | Sim | 열/전력/성능 통합 시뮬레이터 개발 | MED | 미시작 |
| 11 | Chiplet | Multi-chiplet 변형 (2x HEXA-3D on interposer) | LOW | 미시작 |
| 12 | Integration | Level 4 (HEXA-PHOTON) 광 인터커넥트 포트 예약 | LOW | 미시작 |
...
```
  ...
  │                                                             │
  │  [ ] TSV 288/mm^2 at sigma*tau=48um pitch — TSMC SoIC 로드맵│
  │  [ ] 12-hi HBM4 stack thermal budget < 85 degC              │
  │  [ ] Microfluidic cooling 300W capacity 달성                │
  │  [ ] Cu-Cu hybrid bonding 5um pad 양산성                    │
  │  [ ] Combined yield > 50% (KGD + redundancy)               │
  │  [ ] Vertical bandwidth 100 TB/s sustained                  │
  │  [ ] PIM layer N5 + Compute layer N2 mixed-process bonding │
  │  [ ] Total package height < 2.5 mm (소켓 호환)              │
  └────────────────────────────────────────────────────────────┘
```
