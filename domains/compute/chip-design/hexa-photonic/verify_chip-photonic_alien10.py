...
def tp_p_a2_casimir():
    ...
    return {"id":"TP-PHOTONIC-A2","name":"Casimir d⁻⁴ (exp=τ)",
            "d_m":d,"pressure_Pa":pressure,"exponent_eq_tau":4==TAU,
            "PASS":pressure>0 and 4==TAU,"alien_tgt":10,"closure":10}

def tp_p_a3_stefan_boltzmann():
    T = 1000.0
    j = SIGMA_SB * T**4
    return {"id":"TP-PHOTONIC-A3","name":"Stefan-Boltzmann T⁴ (exp=τ)",
...
def tp_p_a10_holographic():
    ...
    return {"id":"TP-PHOTONIC-A10","name":"Holographic A/(4ℓ_p²)",
            ...
            "S_bits_per_m2":S/log(2),
            "PASS":S>0,"alien_tgt":10,"closure":7}

def tp_p_a11_speed_of_light():
    c_si = 299792458
    return {"id":"TP-PHOTONIC-A11","name":"c SI 2019 EXACT",
            "c_m_per_s":c_si,"matches":C==c_si,
...
def tp_p_a12_fourier_limit():
    ...
    bw, dt = 1e12, 1e-12
    product = bw * dt
    return {"id":"TP-PHOTONIC-A12","name":"Fourier BW·Δt",
            "bw_Hz":bw,"dt_s":dt,"product":product,
            "floor_1_over_4pi":floor,
            "above_floor":product >= floor,
            "PASS":product >= floor,"alien_tgt":10,"closure":7}
