...
- `vt_6tier_terminal.md` — 6 계층 (RENDER/INPUT/STREAM/CAPABILITY/OS-BRIDGE/AI-NATIVE) 프로토콜 스펙, σ=12 채널 multiplex, 6 byte header
- VOID 경로: `/Users/ghost/Dev/void/` (app/ai/core/platform 트리)
- 분담:
  - tier 1 RENDER + tier 6 AI-NATIVE → VOID 호스트 프론트엔드
  - tier 2 INPUT + tier 3 STREAM + tier 4 CAPABILITY + tier 5 OS-BRIDGE → hexa-macos SoC 펌웨어
- n=6 정렬: 6 tier, 12 channel, 6 byte header, τ=4 레이턴시, φ·sopfr=10 토큰, 4 OS 서브시스템 → 9/9 EXACT
