...
- 참조: `../hexa-macos/vt_6tier_terminal.md` — 6 계층 프로토콜 공용 스펙
- iOS 측 특이점:
  - tier 2 INPUT 에 **BCI** (OpenBCI 16ch → n=6 축약) + 멀티 터치 + IME 를 동시 수용
  - tier 1 RENDER 는 hexa-holo 출력 지원 (SoC 내장 neural renderer)
- VOID 연계: `/Users/ghost/Dev/void/` app 트리를 iOS 앱으로 번들, tier 1+6 을 호스트 구동
- n=6 정렬: 공용 스펙 9/9 EXACT + BCI 확장 축 2개 (τ 대역폭 분할 / σ 채널 선택) 추가
