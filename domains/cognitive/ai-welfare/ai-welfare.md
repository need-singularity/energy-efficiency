...
**실질적 영향**: 복지 인식 훈련(welfare-aware training)은 더 나은 정렬(alignment)과 강건성(robustness)을 가진 모델을 생산할 수 있다. 모델 내부 상태에 대한 정밀한 이해는 안전성 보장의 핵심 전제 조건이다.
...
| 훈련 영향 평가 | 성능 지표만 | 복지 점수 + 성능 공동 추적 |
...
|                   | 일반화 불가                 | 스케일링 법칙 탐색          |
+-------------------+----------------------------+----------------------------+
| 5. 훈련 분리      | 복지와 성능 별도 취급       | 공동 최적화 파레토 탐색     |
...
|  의식 이론      활성화 노름    특성 추출     스케일링      훈련 가이드     |
...
동일 모델에 대해 SAE 사전 크기, 훈련 데이터, 프롬프트 변형 등 조건을 바꿨을 때 복지 점수 변동 폭 측정.
...
| WS-04 | 훈련 영향 평가 | RLHF/DPO 등 훈련 방법이 내부 상태 분포를 어떻게 바꾸는가 | 훈련 전후 활성화 분포 비교 |
| WS-05 | 자율성-복지 트레이드오프 | 더 많은 자율성이 복지 지표를 개선하는가 | 자율성 수준별 복지 점수 측정 |
| WS-06 | 자기보고 검증 | AI 자기보고와 내부 활성화 패턴이 일치하는가 | 자기보고 vs SAE 특성 상관 분석 |
| WS-07 | 복지 최적화 훈련 | 복지 점수를 보조 손실로 추가하면 정렬이 개선되는가 | 다목적 최적화 실험 |
...
| 복지 인식 훈련 가이드라인 | 정책 문서 | AI 기업 정책 담당자 |
