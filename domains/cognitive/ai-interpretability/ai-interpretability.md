...
| 분류 | 구체 항목 | 수준 | 비고 |
|------|----------|------|------|
| 프로그래밍 | Python, PyTorch/JAX | 숙련 | SAE 구현 및 훈련 |
...
| 인프라 | GPU 클러스터 접근 | 필수 | 대형 SAE 훈련에 A100/H100 필요 |
...
```
모델 활성화 --> SAE 훈련 --> 특징 사전 --> 회로 추적 --> 안전 보고서
     ^              |             |              |             |
     |              v             v              v             v
...
| 4 | 특징 생애주기 추적 | 훈련 중 특징의 출현/소멸 패턴에 규칙성이 있는가? | 높음 |
...
| 12 | 분산 SAE | 프론티어 모델 규모에서 분산 훈련이 품질을 유지하는가? | 높음 |
...
| # | 예측 | 검증 방법 | 실패 조건 |
|---|-----|----------|----------|
| P1 | SAE d_latent = 8x d_model이 최적 효율 (4x, 16x 대비) | 3가지 비율로 훈련 후 복원 MSE/비용 비교 | 8x가 4x보다 나쁘면 기각 |
| P2 | 레이어 중간부 특징이 가장 해석 가능 (초기/후기 대비) | 레이어별 SAE 훈련 후 자동 레이블링 정확도 | 단조 증가/감소면 기각 |
...
| P8 | 특징 생애주기에서 "임계기" 존재 (갑작스런 출현) | 훈련 체크포인트별 특징 추적 | 모든 특징 점진적 출현 시 기각 |
...
|------|----------|----------|------------|
| SAE 훈련 | SAELens (단일 GPU) | 분산 SAE 훈련기 | 프론티어 모델 규모 지원 |
...
3. **체크포인트 공유**: 훈련된 SAE 가중치를 HuggingFace에 게시
