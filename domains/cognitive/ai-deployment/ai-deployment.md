...
Anthropic Fellows 2026 연구 도메인. 4대 기둥: 훈련 안전, 추론 안전, 배포 프로토콜, 프롬프트 방어.
...
**한 문장**: 훈련->추론->배포->프롬프트 방어 전 단계 안전 프레임워크로 연구-현실 간극을 메운다.
...
```
+----------------+----------------+------------------+------------------+
|  훈련 안전 (4) |  추론 안전 (4) |  배포 프로토콜(8)|  프롬프트 방어(8)|
+----------------+----------------+------------------+------------------+
...
```
훈련: [안전 데이터] -> [커리큘럼 학습] -> [그래디언트 수술] -> [안전 모델]
...
### §7.10 COUNTER (정직한 한계)
- 제로데이 공격: 훈련 분포 밖 신규 패턴 사전 탐지 불가능
